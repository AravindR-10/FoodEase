...
public class DeliveryServiceImpl implements DeliveryService {
    ...
    public DeliveryDTO createDelivery(DeliveryDTO deliveryDTO) {
        ...
        User partner = userRepository.findById(deliveryDTO.getPartnerId())
                .orElseThrow(() -> new ResourceNotFoundException("Delivery partner not found with ID: " + deliveryDTO.getPartnerId()));
    ...
    public List<DeliveryDTO> getDeliveriesByPartnerId(Long partnerId) {
        ...
        return deliveries.stream()
                .map(delivery -> new DeliveryDTO(delivery.getId(), delivery.getOrder().getId(), delivery.getPartner().getId(),
                        delivery.getStatus().name(), delivery.getLastUpdate()))
